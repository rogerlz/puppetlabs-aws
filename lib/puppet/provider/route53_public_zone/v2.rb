...
Puppet::Type.type(:route53_public_zone).provide(:v2, :parent => PuppetX::Puppetlabs::Aws) do
  confine feature: :aws
  ...
  def exists?
    Puppet.info("Checking if public zone #{name} exists")
    @property_hash[:ensure] == :present
  end

  def create
    reference = SecureRandom.hex
    Puppet.info("Creating public zone #{name} with #{reference}")
  ...
  def destroy
    Puppet.info("Deleting public zone #{name}")
